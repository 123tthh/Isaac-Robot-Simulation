...
```text
legacy Trajectory -> projects/teleoperation
...
```bash
./scripts/project_control.sh sim1-demo --speed 0.5
./scripts/project_control.sh sim1-demo --loop
```
...
```bash
cd /path/to/Isaac-Robot-Simulation/projects/teleoperation/sim1
...
```text
projects/teleoperation/sim1/trace_data/lerobot_v30/
```
...
/workspace/projects/teleoperation/sim1
...
`r1_workcell/configuration/physics.usd</visuals/base_link>` 的引用
仍可能报告无法解析。因此 `assets/scenes/r1_workcell` 依赖目录暂时
...
```python
exec(open('scripts/repair_scene_cameras.py', encoding='utf-8').read())
```
...
- `scripts/project_control.sh`：宿主机统一入口
- `projects/teleoperation/sim1/game_control.sh`：SIM1 模式入口
- `projects/teleoperation/sim1/TRACE_DATA_FORMAT.md`：轨迹字段和转换格式
