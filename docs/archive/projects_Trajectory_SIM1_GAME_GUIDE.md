...
- `${PROJECT_ROOT}/projects/teleoperation/sim1/graph/ALL_Graph.md`
- `${PROJECT_ROOT}/projects/teleoperation/sim1/graph/Gripper_Control_Graph.md`
- `${PROJECT_ROOT}/data/ros2_log/scripts/夹爪调试/00整理版/POS控制/SCRIPTS NODE/script_gripper_v4.5.py`
...
终端 A：启动 Isaac Sim 5.1.0，并明确加载整理后的 `workcell.usd`。启动后在 Isaac Sim GUI 中按 **Play**；未按 Play 时，ROS 2 相机 topic 和仿真控制服务都不会工作。
...
source ${PROJECT_ROOT}/projects/ros2_ws/install/setup.bash
...
```python
exec(open('${PROJECT_ROOT}/scripts/repair_scene_cameras.py', encoding='utf-8').read())
```

脚本默认只读，报告写入
`${PROJECT_ROOT}/reports/615scene_camera_script_editor_20260724.md`。
报告确认存在缺失 `NodeGraphNodeAPI` 节点后，才将脚本顶部
`APPLY_REPAIR = False` 改为 `True`，再次执行；脚本会先备份 USD，再写回
`workcell.usd`。写回后重新打开场景并按 Play。
...
```bash
cd ${PROJECT_ROOT}/projects/teleoperation/sim1
source ${PROJECT_ROOT}/projects/ros2_ws/install/setup.bash
./game_control.sh teach --open-camera-grid --input-mode pygame \
  --record-cameras \
  --camera-depth-save-every 1
...
如果你确实想在 Isaac Sim GUI 内看机载相机画面，可参考本地 Isaac Sim 文档 `https://docs.isaacsim.omniverse.nvidia.com/5.1.0/robot_setup_tutorials/tutorial_gui_camera_sensors.md`：
...
```text
${PROJECT_ROOT}/projects/ros2_ws/src/robot/config/ros2_control/ocs2_controllers.yaml
```
...
当前工程按 `script_gripper_v4.5.py`：`1.0` 打开，`0.0` 闭合/保持。若 Isaac 中实际方向不一致，应先检查 Script Node 是否加载的是 `${PROJECT_ROOT}/data/ros2_log/scripts/夹爪调试/00整理版/POS控制/SCRIPTS NODE/script_gripper_v4.5.py`，并确认 Graph 连线是 `position_cmds -> positionCommand`。
...
如果服务不存在，确认 Isaac Sim 已启用 `isaacsim.ros2.sim_control`，并确认当前终端已 source `${PROJECT_ROOT}/projects/ros2_ws/install/setup.bash`。
...
```text
base_control/script_node.py
```

若场景中换成调试版 ScriptNode：

```text
${PROJECT_ROOT}/data/ros2_log/scripts/base_control/script_node_base_arc_debug_v2.py
```
