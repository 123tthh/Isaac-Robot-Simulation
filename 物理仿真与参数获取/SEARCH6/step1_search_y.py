...
Y_SEARCH  = np.arange(-0.050, 0.050 + 0.0001, 0.001)  # -50~+50mm 步进1mm
N_TRIALS  = 3
...
async def main():
    ...
    log = {
        "config": {
            "step": "step1_Y", "SUCCESS_DX_m": SUCCESS_DX,
            "MU_ROLLER_S_BASE": MU_ROLLER_S_BASE,
            "PITCH_BASE": PITCH_BASE, "Yaw_fixed_deg": 0.0,
            "Y_range_mm": [round(Y_SEARCH[0]*1000,1), round(Y_SEARCH[-1]*1000,1)],
            "Y_step_mm": 1.0, "N_TRIALS": N_TRIALS,
        },
    ...
    for y in Y_SEARCH:
        ...
        print(f"  Y={y_mm:6.1f}mm → {rate:.0%} {s}")
