...
Local documentation references:
  https://docs.isaacsim.omniverse.nvidia.com/5.1.0/development_tools/omniverse_script_editor.md
  https://docs.isaacsim.omniverse.nvidia.com/5.1.0/py/source/extensions/isaacsim.core.utils/docs/api.md
  https://docs.isaacsim.omniverse.nvidia.com/5.1.0/replicator_tutorials/tutorial_replicator_amr_navigation.md
  https://docs.isaacsim.omniverse.nvidia.com/5.1.0/replicator_tutorials/tutorial_replicator_getting_started.md
...
PROJECT_ROOT = Path(
    os.environ.get("ISAAC_OCS_PROJECT_ROOT", str(Path(__file__).resolve().parents[1]))
).expanduser().resolve()
EXPECTED_SCENE = Path(
    os.environ.get(
        "ISAAC_USD_PATH",
...
async def run_when_stage_ready() -> dict:
    try:
        # Extension registration is completed on the next Kit update before
        # opening a stage that instantiates ROS and image graph nodes.
        await omni.kit.app.get_app().next_update_async()
...
STARTUP_TASK = asyncio.ensure_future(run_when_stage_ready())
