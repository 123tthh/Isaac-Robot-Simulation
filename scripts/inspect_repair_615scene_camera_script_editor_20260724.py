...
REPORT_PATH = PROJECT_ROOT / "reports/615scene_camera_script_editor_20260724.md"
BACKUP_DIR = PROJECT_ROOT / "backups/615scene_camera_repair_20260723"
...
async def run() -> None:
    ...
    if stage is None or not stage.GetRootLayer().identifier.endswith("615scene.usd"):
        context.open_stage(str(USD_PATH))
        for _ in range(120):
            await omni.kit.app.get_app().next_update_async()
        stage = context.get_stage()
    if stage is None or not stage.GetRootLayer().identifier.endswith("615scene.usd"):
        raise RuntimeError("615scene.usd did not finish loading; run the script again after opening it")
    report_lines = [
    ...
    if missing:
        ...
        if APPLY_REPAIR:
            ...
            backup = BACKUP_DIR / f"615scene.usd.before_script_editor_{_dt.datetime.now():%Y%m%d_%H%M%S}"
            shutil.copy2(USD_PATH, backup)
            source = Sdf.Layer.FindOrOpen(str(USD_PATH))
            if source is None:
                raise RuntimeError(f"cannot open {USD_PATH}")
            temp = Path("/tmp/615scene_script_editor_camera_repair.usda")
