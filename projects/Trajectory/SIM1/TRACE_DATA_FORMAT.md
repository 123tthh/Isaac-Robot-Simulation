...
本文说明 `~/Trajectory/SIM1` 下示教、清洗、标注、转换后的数据格式。
...
底盘控制语义以 Isaac Sim `Base_Drive_Graph` 当前使用的 `底盘差速驱动/script_node.py` 为准。该 ScriptNode 使用 `/sim1/base_diff_cmd` 的 `linear.x` 控制直行/后退，用 `linear.x = 0` 时的 `angular.z` 正负触发左/右 90°圆弧转弯。
