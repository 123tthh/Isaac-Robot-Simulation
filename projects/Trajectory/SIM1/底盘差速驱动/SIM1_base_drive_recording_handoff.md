...
```bash
mkdir -p ~/Trajectory/SIM1/trace_data/base_bag
...
ros2 bag record \
  ...
  /joint_states \
  /clock \
  -o ~/Trajectory/SIM1/trace_data/base_bag/base_drive_$(date +%Y%m%d_%H%M%S)
```
...
```text
~/Trajectory/SIM1/base_drive_recorder.py
```

输出目录：

```text
~/Trajectory/SIM1/trace_data/base_raw/
```
...
请在 ~/Trajectory/SIM1 中新增底盘轨迹记录能力，先不要改 OCS2 和夹爪逻辑。
