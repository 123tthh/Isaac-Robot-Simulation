...
namespace gz_ros2_control
{
    ...
    // Jazzy no longer has the Humble ResourceManager default constructor or
    // load_urdf().  Keep the Gazebo-specific initialization hook used by the
    // upstream Jazzy gz_ros2_control implementation.
    ...
    public:
        GZResourceManager(
            ...
            : hardware_interface::ResourceManager(
              ...
              node_(node),
              ecm_(&ecm),
              enabled_joints_(std::move(enabled_joints))
        {
        }
            ...
            const unsigned int update_rate) override
        {
            ...
            int simulation_update_rate = static_cast<int>(update_rate);
            ...
            for (const auto& individual_hardware_info : hardware_info)
            {
                ...
                std::unique_ptr<GazeboSimSystemInterface> gz_sim_system;
                std::scoped_lock guard(
                    resource_interfaces_lock_,
                    claimed_command_interfaces_lock_);

                try
                {
                    gz_sim_system =
                        std::unique_ptr<GazeboSimSystemInterface>(
        ...
        sim::EventManager&)
    {
        ...
        std::unique_ptr<hardware_interface::ResourceManager> resource_manager_ =
            std::make_unique<GZResourceManager>(
                this->dataPtr->node_,
                _ecm,
                enabledJoints);
#else
        // Humble resource manager setup.
