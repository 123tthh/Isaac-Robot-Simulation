...
namespace arms_controller_common
{
    ...
    void StateMoveJ::enter()
    {
        ...
        {
            auto value = i.get().get_value();
            start_pos_.push_back(value);
            current_vel_.push_back(0.0);
        }
    ...
    void StateMoveJ::run(const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/)
    {
        ...
        if (interpolation_type_ == InterpolationType::CRITICAL_DAMPING)
        {
            double dt = 1.0 / controller_frequency;
            for (size_t i = 0; i < ctrl_interfaces_.joint_position_command_interface_.size() &&
                i < target_pos_.size() && i < start_pos_.size(); ++i)
            {
                double current_pos = ctrl_interfaces_.joint_position_command_interface_[i].get().get_value();
                // Critical damping filter: a = omega^2 * (target - pos) - 2 * omega * vel
                double accel = filter_omega_ * filter_omega_ * (target_pos_[i] - current_pos) -
                               2.0 * filter_omega_ * current_vel_[i];

                current_vel_[i] += accel * dt;
                double next_pos = current_pos + current_vel_[i] * dt;

                if (use_prefix_filter_ && i < joint_mask_.size() && !joint_mask_[i])
                {
                    // Hold this joint: do not update next_pos (keep at start_pos_ handled in initialization or maintain current)
                    continue;
                }
                ctrl_interfaces_.joint_position_command_interface_[i].get().set_value(next_pos);
            }
        }
        else
        {
        ...
        if (ctrl_interfaces_.control_mode_ == ControlMode::MIX && gravity_compensation_)
        {
            ...
            {
                auto value = ctrl_interfaces_.joint_position_command_interface_[i].get().get_value();
                interpolated_positions.push_back(value);
            }
    ...
    void StateMoveJ::setInterpolationType(const std::string& type)
    {
        ...
        if (t != "linear" && t != "tanh" && t != "critical_damping" && t != "filter")
        {
        ...
        RCLCPP_INFO(logger_, "MoveJ interpolation type set to '%s'", type.c_str());
    }
    ...
    void StateMoveJ::setFilterOmega(double omega)
    {
        if (omega <= 0.0 || !std::isfinite(omega))
        {
            RCLCPP_WARN(logger_, "Invalid movej filter omega %.3f, keeping %.3f", omega, filter_omega_);
            return;
        }
        filter_omega_ = omega;
    }
