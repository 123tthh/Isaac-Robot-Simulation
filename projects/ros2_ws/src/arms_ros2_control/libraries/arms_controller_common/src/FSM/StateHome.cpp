...
namespace arms_controller_common
{
    ...
    void StateHome::run(const rclcpp::Time& /*time*/, const rclcpp::Duration& /*period*/)
    {
        ...
        if (interpolation_type_ == InterpolationType::CRITICAL_DAMPING)
        {
                ...
                i < current_target_.size() && i < start_pos_.size(); ++i)
            {
                ...
                double accel = filter_omega_ * filter_omega_ * (current_target_[i] - current_pos) -
    ...
    void StateHome::setFilterOmega(double omega)
    {
        if (omega <= 0.0 || !std::isfinite(omega))
        {
            RCLCPP_WARN(logger_, "Invalid home filter omega %.3f, keeping %.3f", omega, filter_omega_);
            return;
        }
        filter_omega_ = omega;
    }
