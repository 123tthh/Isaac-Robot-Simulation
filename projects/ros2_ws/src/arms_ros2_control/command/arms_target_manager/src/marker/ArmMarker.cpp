...
namespace arms_ros2_control::command
{
          ...
          , last_subscription_update_time_(node_->now())
    {
        ...
        current_target_subscription_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>(
            ...
            [this](const geometry_msgs::msg::PoseStamped::ConstSharedPtr msg)
            {
                ...
                // current_target comes from the active controller. In OCS2
                // interactive mode it may lag behind a freshly dragged marker,
                // so keep the frame_id but do not let it pull the marker pose
                // back over the user's target.
    ...
    {
        const bool needs_initial_sync = !initial_target_synced_;

        // 先检查是否允许自动更新（只有在非禁用状态下才更新 pose_）
        if (!needs_initial_sync && state_check_callback_ && !state_check_callback_())
        {
        ...
        auto now = node_->now();
        auto time_since_last = (now - last_subscription_update_time_).seconds();
        if (!needs_initial_sync && time_since_last < 1.0 / 30.0)  // 30Hz = 1/30秒
        {
            return;  // 跳过此次更新
        }
        last_subscription_update_time_ = now;
        ...
        if (source_frame_id.empty())
        {
            source_frame_id = control_base_frame_;
        }
        ...
        {
            publishTargetPose(true, true);
            initial_target_synced_ = true;
            RCLCPP_INFO(
                node_->get_logger(),
                "Initialized %s target from first current pose in frame '%s'",
                getMarkerName().c_str(),
