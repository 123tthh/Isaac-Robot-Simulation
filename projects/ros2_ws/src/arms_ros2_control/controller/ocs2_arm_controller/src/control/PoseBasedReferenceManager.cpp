...
namespace ocs2::mobile_manipulator
{
    ...
    {
        // 使用 moveL_duration_ 或 time_from_start 作为插值时间
        const double actual_duration = (time_from_start < 0.0) ? moveL_duration_ : time_from_start;

        // 采样间隔（秒）
        constexpr double kSampleInterval = 0.02; // 0.02秒一个采样点（50Hz）
    ...
    void PoseBasedReferenceManager::leftPoseStampedPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
    {
        // 保存上一帧缓存（用于插值起点）
        const vector_t previous_left_target_state = left_target_state_;

        // 从stamp获取插值时间
        double interpolation_time = rclcpp::Time(msg->header.stamp).seconds();

        // 验证时间合理性：如果时间 <= 0 或 > 10秒，则使用默认时长 (-1.0)
        if (interpolation_time <= 0.0 || interpolation_time > 10.0)
        {
        ...
        // 更新缓存到新目标
        vector_t target_state = vector_t::Zero(7);
        target_state(0) = msg->pose.position.x;
        target_state(1) = msg->pose.position.y;
        target_state(2) = msg->pose.position.z;
        target_state(3) = msg->pose.orientation.x;
        target_state(4) = msg->pose.orientation.y;
        target_state(5) = msg->pose.orientation.z;
        target_state(6) = msg->pose.orientation.w;
        left_target_state_ = target_state;
        ...
        if (dual_arm_mode_)
        {
            ...
            updateTrajectory(previous_left_target_state, previous_right_target_state, interpolation_time);
        }
        else
        {
            // 单臂模式：只更新左臂，传递零向量作为右臂占位符（不会被使用）
            updateTrajectory(previous_left_target_state, vector_t::Zero(7), interpolation_time);
        }
    ...
    void PoseBasedReferenceManager::rightPoseStampedPoseCallback(const geometry_msgs::msg::PoseStamped::SharedPtr msg)
    {
        // 保存上一帧缓存（用于插值起点）
        const vector_t previous_left_target_state = left_target_state_;
        const vector_t previous_right_target_state = right_target_state_;
    ...
    void PoseBasedReferenceManager::processPoseStamped(
        ...
        std::function<void(geometry_msgs::msg::PoseStamped::SharedPtr)> callback)
    {
        ...
        try
        {
            auto transformed_pose = std::make_shared<geometry_msgs::msg::PoseStamped>();
    ...
    {
        processPoseStamped(msg, [this](geometry_msgs::msg::PoseStamped::SharedPtr pose_msg)
            {
