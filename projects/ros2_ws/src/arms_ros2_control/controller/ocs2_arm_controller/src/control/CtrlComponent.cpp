...
namespace ocs2::mobile_manipulator
{
    ...
    void CtrlComponent::updateObservation(const rclcpp::Time& time)
    {
        ...
        {
            auto value = ctrl_interfaces_.joint_position_state_interface_[i].get().get_value();
            observation_.state[i] = value;
        }
    ...
    void CtrlComponent::evaluatePolicy(const rclcpp::Time& time)
    {
        ...
        try
        {
            ...
            // Apply Second-Order Critical Damping Filter to smooth MPC output
            if (mpc_filter_enabled_)
            {
                double dt = 1.0 / ctrl_interfaces_.frequency_;
                if (filter_vel_.size() != future_state.size())
                {
                    filter_vel_ = vector_t::Zero(future_state.size());
                    last_filter_pos_ = observation_.state;
                }

                vector_t next_state = future_state;
                for (size_t i = 0; i < static_cast<size_t>(future_state.size()); ++i)
                {
                    // a = omega^2 * (target - pos) - 2 * omega * vel
                    double accel = mpc_filter_omega_ * mpc_filter_omega_ * (future_state(i) - last_filter_pos_(i)) -
                                   2.0 * mpc_filter_omega_ * filter_vel_(i);
                    filter_vel_(i) += accel * dt;
                    next_state(i) = last_filter_pos_(i) + filter_vel_(i) * dt;
                }
                last_filter_pos_ = next_state;
            ...
            if (ctrl_interfaces_.control_mode_ == ControlMode::POSITION)
            {
                ...
                if (interface_->shouldUpdateDynamicNullspaceReferenceFromPolicy())
                {
                    interface_->setDynamicNullspaceReference(future_state);
                }
            }
        ...
        catch (const std::exception& e)
        {
            ...
            {
                auto value = ctrl_interfaces_.joint_position_state_interface_[i].get().get_value();
                current_positions(i) = value;
            }

            double dt = 1.0 / ctrl_interfaces_.frequency_;
            vector_t fallback_action = current_positions;
            for (size_t i = 0; i < joint_names_.size() && i < static_cast<size_t>(optimized_input_.size()); ++i)
            {
                ...
                fallback_action(i) = new_position;
            }
            cached_last_action_ = fallback_action;
            last_execute_time_ = time;
            if (interface_->shouldUpdateDynamicNullspaceReferenceFromPolicy())
            {
                interface_->setDynamicNullspaceReference(fallback_action);
            }
        }
    ...
    void CtrlComponent::resetMpc()
    {
        ...
        // Reset filter state
        filter_vel_ = vector_t::Zero(observation_.state.size());
        last_filter_pos_ = observation_.state;
        ...
        if (interface_->isDynamicNullspaceCostEnabled())
        {
            interface_->setDynamicNullspaceReference(observation_.state);
            RCLCPP_INFO(node_->get_logger(),
                        "Updated dynamic null-space reference from current OCS2 entry state");
        }
