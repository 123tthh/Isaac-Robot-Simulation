    ...
    void StateOCS2::enter()
    {
        ...
        if (ctrl_interfaces_.control_mode_ == ControlMode::MIX)
        {
            if (ctrl_interfaces_.pd_gains_.size() >= 2)
            {
                ...
                {
                    auto& kp_interface = ctrl_interfaces_.joint_kp_command_interface_[i].get();
                    kp_interface.set_value(kp);
                }
                for (size_t i = 0; i < ctrl_interfaces_.joint_kd_command_interface_.size(); ++i)
                {
                    auto& kd_interface = ctrl_interfaces_.joint_kd_command_interface_[i].get();
                    kd_interface.set_value(kd);
                }
            }
            else
            {
