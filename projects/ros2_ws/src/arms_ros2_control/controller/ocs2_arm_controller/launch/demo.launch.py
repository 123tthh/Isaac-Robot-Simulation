...
def generate_launch_description():
    ...
    robot_name_arg = DeclareLaunchArgument("robot", default_value="cr5", description="Robot name (arx5, cr5, etc.)")

    robot_type_arg = DeclareLaunchArgument(
        "type", default_value="", description="Robot type (x5, r5, robotiq85, etc.). Leave empty to not pass type parameter to xacro."
    )
    ...
    world_arg = DeclareLaunchArgument("world", default_value="dart", description="Gz sim World (only used when hardware=gz)")

    enable_arms_target_manager_arg = DeclareLaunchArgument(
        "enable_arms_target_manager", default_value="true", description="Enable ArmsTargetManager for interactive pose control"
    )

    enable_gripper_arg = DeclareLaunchArgument(
        "enable_gripper", default_value="true", description="Enable gripper controllers and gripper control panel"
    )
