...
def feature(dtype: str, shape: list[int], names: list[str] | None, fps: int) -> dict[str, object]:
    return {"dtype": dtype, "shape": shape, "names": names if names else None, "fps": fps}
...
def build_dataset_rows(
    ...
    # LeRobot timestamps and encoded frames must share an exact fixed-rate grid.
    times = [float(row["wall_time"]) for row in rows]
    if any(b < a for a, b in zip(times, times[1:])):
        raise ValueError("Trace wall timestamps are not monotonic")
    grid = times[0] + np.arange(int((times[-1] - times[0]) * fps) + 1) / fps
    sampled = []
    for t in grid:
        j = min(bisect_left(times, t), len(times) - 1)
    ...
    timestamps = [i / fps for i in range(len(rows))]
...
def copy_selected_videos(
    ...
    for camera_name, video_key in CAMERA_VIDEO_KEYS.items():
        ...
        if not writer.isOpened():
            raise RuntimeError(f"Cannot create video: {dest}")
        source_index = -1
        frame = None
        try:
            for row_index in camera_indices[camera_name]:
                wanted = int(rows[row_index]["frame_index"])
                while source_index < wanted:
                    ok, frame = cap.read()
                    if not ok:
                        raise RuntimeError(f"Video shorter than timestamp index: {source}, frame {wanted}")
                    source_index += 1
                if frame is None:
                    raise RuntimeError(f"No frame selected: {source}")
                writer.write(frame)
        finally:
            writer.release()
            cap.release()
    # Preserve metric float32 depth as lossless auxiliary files. These are not RGB
    # LeRobot video features; the timestamp index documents their alignment.
    depth_rows = []
    for camera_name, rows in camera_rows_by_name.items():
        if not camera_name.endswith("_depth"):
            continue
        for row in rows:
            rel = row.get("relative_path", "")
            if not rel:
                continue
            source = camera_dir / rel
            ...
            depth_rows.append(dict(row))
    if depth_rows:
        pd.DataFrame(depth_rows).to_parquet(output_root / "depth/index.parquet", index=False)
