...
def main() -> int:
    args = parse_args()
    # Child converters run from the SIM1 directory; resolve a caller-relative
    # input before passing it to those subprocesses.
    raw_trace = args.raw_trace.expanduser().resolve()
    if not raw_trace.exists():
        raise FileNotFoundError(raw_trace)

    run_id = raw_trace.with_suffix("").name
    if run_id == "trace":
        run_id = raw_trace.parent.name
