"""
step5_search_friction.py  ── Isaac Sim 5.1 · Script Editor（完全自包含）
═══════════════════════════════════════════════════════════════════
Step 5 (新位置): μ_roller_s 扫描 → 最终 safe_pose.json

  • 固定: Y/Yaw/Pitch/Mass 为前四步中点
  • 扫描: μ_roller_s 0.020 ~ 0.100，步进 0.005
  • Z 偏移 = Pitch 固定值对应的补偿高度
  • Yaw 锁死检测 + rail_lock 当失败

  若 safe_pose_mass.json 不存在，fallback 使用默认 1.0kg
═══════════════════════════════════════════════════════════════════
"""
...
MU_SEARCH = np.arange(0.020, 0.100 + 0.001, 0.005)
N_TRIALS  = 5
...
MASS_INPUT  = os.path.join(RESULT_DIR, "safe_pose_mass.json")
...
async def main():
    ...
    print("═" * 62)
    print("[Step 5 / 新] μ_roller_s 扫描 → 最终 Safe Pose Set")
    ...
    mass_range = None
    if os.path.exists(MASS_INPUT):
        with open(MASS_INPUT) as f:
            mass_range = json.load(f).get("safe_mass_range_kg")
    if mass_range:
        MASS_FIXED = (mass_range[0] + mass_range[1]) / 2.0
        print(f"  ✓ 加载 Step4 质量区间: {mass_range} kg → 中点 {MASS_FIXED:.2f}kg")
    else:
        MASS_FIXED = DEFAULT_MASS_KG
        print(f"  ⚠ 未找到 step4 结果，fallback 使用默认 {DEFAULT_MASS_KG}kg")
    ...
    print(f"  基准 Mass: {MASS_FIXED:.2f} kg")
    print(f"  μ 范围   : {MU_SEARCH[0]:.3f} ~ {MU_SEARCH[-1]:.3f}"
          f"  步进 0.005  共 {len(MU_SEARCH)} 点")
    ...
    log = {
        "config": {
            ...
            "Mass_fixed_kg": round(MASS_FIXED, 2),
            "Mass_input_range_kg": mass_range,
            "MU_range": [round(float(MU_SEARCH[0]),4),
                         round(float(MU_SEARCH[-1]),4)],
            "MU_step": 0.005,
            "N_TRIALS": N_TRIALS,
        },
        "trials": [], "summary": [],
    ...
    for mu_s in MU_SEARCH:
        ...
        print(f"  μ={mu_r:.4f} (eff={mu_r*PALLET_MU_S:.4f}) → {rate:.0%}  "
              f"(ok={successes} lock={locks} stuck={stucks}) {s}")
        log["summary"].append({
            "mu_roller_s": mu_r, "rate": round(rate, 3),
    ...
    log["final_safe_pose"] = {
        "position": {
            ...
            "Z_m"       : PAL_Z0_ACTUAL + Z_FIXED,
            "Z_offset_m": round(Z_FIXED, 5),
        },
        "orientation": {
            "Roll_deg"       : 0.0,
        ...
        "mass": {
            "mass_range_kg": mass_range,
            "mass_used_kg" : round(MASS_FIXED, 2),
            "dim_m"        : [PALLET_DIM_X, PALLET_DIM_Y, PALLET_DIM_Z],
        },
        "friction": {
    ...
    print(f"    Mass  : {mass_range or '(step4 未运行)'} kg")
