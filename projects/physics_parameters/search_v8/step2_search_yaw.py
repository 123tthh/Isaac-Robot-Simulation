"""
step2_search_yaw.py  ── Isaac Sim 5.1 · Script Editor（完全自包含）
═══════════════════════════════════════════════════════════════════
Step 2: 读 Step1 的 Y 安全区间，固定 Y=区间中点、μ、Pitch=4°
        只扫描 Yaw: -15° ~ +15°，步进 1°

本版改动（v2，与 step1 同步）：
  ★ Z_OFFSET = 1.24mm（贴滚轮释放）
  ★ 导轨侧面高摩擦材质一次性绑定 (μs=0.60 / μd=0.50)
  ★ Yaw 锁死检测：|Δyaw| > 3° 持续 ≥ 0.5s → rail_lock（当失败）
  ★ rate 只统计 success，rail_lock 和 stuck 都是失败

注意：
  本 step 输入的 yaw_deg 就是目标 Yaw（不是 0），所以 yaw_drift 定义为
  settle 后实际 yaw - 输入目标 yaw。偏离目标太多说明接触导轨被推开了。
...
class SceneHandle:
    ...
    def _ensure_rail_friction(self):
        ...
        print(f"    ✓ 已绑定导轨摩擦材质到 {bound}/4 根  μs={MU_RAIL_S} μd={MU_RAIL_D}")
...
YAW_SEARCH = np.arange(-15.0, 15.0 + 0.01, 1.0)
N_TRIALS   = 3
...
async def main():
    ...
    print("═" * 62)
    print("[Step 2] Yaw 扫描 (v2)")
    print(f"  Z 偏移  : {Z_OFFSET_M*1000:.2f} mm (贴滚轮)")
    print(f"  锁死阈值: |Δyaw| > {YAW_LOCK_THRESHOLD}° 持续 > {YAW_LOCK_DURATION}s")
    print("═" * 62)

    if not os.path.exists(Y_INPUT):
        print(f"✗ 找不到 {Y_INPUT}，请先运行 step1"); return
    ...
    print(f"  Yaw 范围: {YAW_SEARCH[0]:+.0f} ~ {YAW_SEARCH[-1]:+.0f}° 步进1°"
          f"  共 {len(YAW_SEARCH)} 点")
    ...
    log = {
        "config": {
            "step": "step2_Yaw_v2",
            "SUCCESS_DX_m": SUCCESS_DX, "Z_OFFSET_m": Z_OFFSET_M,
            ...
            "Yaw_range_deg": [round(float(YAW_SEARCH[0]),1),
                              round(float(YAW_SEARCH[-1]),1)],
            "Yaw_step_deg": 1.0,
            "N_TRIALS": N_TRIALS,
        },
    ...
    for yaw in YAW_SEARCH:
        ...
        if rate >= 0.8:
            ...
            s = "△mix"
        print(f"  Yaw={yaw_r:+6.1f}° → {rate:.0%}  "
              f"(ok={successes} lock={locks} stuck={stucks}) {s}")
        log["summary"].append({
            "Yaw_deg": yaw_r, "rate": round(rate,3),
            "n_success": successes, "n_rail_lock": locks, "n_stuck": stucks,
        })
        save()

    values = [round(float(w),1) for w in YAW_SEARCH]
    log["safe_range_deg"] = extract_safe_range(values, rates, 0.8)
    save()

    print("\n" + "═" * 62)
    if log["safe_range_deg"]:
        print(f"  ✓ Yaw 安全区间: {log['safe_range_deg']}°")
    else:
        print("  ⚠ 未找到 Yaw 安全区间")
        n_lock = sum(1 for t in log["trials"] if t["result"] == "rail_lock")
        print(f"    rail_lock 触发 {n_lock}/{len(log['trials'])} 次")
