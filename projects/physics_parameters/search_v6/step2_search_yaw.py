...
def set_pallet_pose(sh, x, y, z, roll_deg, pitch_deg, yaw_deg):
    ...
    # 2. 构造旋转四元数（按照 XYZ 顺序，与之前 XformCommonAPI 保持一致）
...
YAW_SEARCH = np.arange(-15.0, 15.0 + 0.01, 1.0)  # -15~+15° 步进1°
N_TRIALS   = 3
...
async def main():
    ...
    log = {
        "config": {
            "step": "step2_Yaw", "SUCCESS_DX_m": SUCCESS_DX,
            "MU_ROLLER_S_BASE": MU_ROLLER_S_BASE, "PITCH_BASE": PITCH_BASE,
            "Y_input_range_mm": y_range, "Y_fixed_mm": round(Y_FIXED_MM,1),
            "Yaw_range_deg": [round(YAW_SEARCH[0],1), round(YAW_SEARCH[-1],1)],
            "Yaw_step_deg": 1.0, "N_TRIALS": N_TRIALS,
        },
    ...
    for yaw in YAW_SEARCH:
        ...
        for t in range(N_TRIALS):
            rec = await run_one_trial(sh, y=Y_FIXED_M, yaw_deg=float(yaw),
                pitch_deg=PITCH_BASE, mu_roller_s=MU_ROLLER_S_BASE,
                tag=f"Yaw={yaw_r}° #{t+1}")
            rec["Yaw_deg"] = yaw_r
        ...
        print(f"  Yaw={yaw_r:6.1f}° → {rate:.0%} {s}")
        log["summary"].append({"Yaw_deg": yaw_r, "rate": round(rate,3)})
        save()
